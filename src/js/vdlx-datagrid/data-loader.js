...
function getAutoTableEntities(columnOptions) {
  ...
  entities = entities.concat(
    flatten(
      map(entities, entity => modelSchema.getEntity(entity).getIndexSets())
    )
  );
...
function getScenarios(config, scenarios) {
  ...
  const defaultScenario = isUndefined(config.scenario)
    ? scenarios[0]
    : findScenario(scenarios, config.scenario);
...
function withScenarioData(config$) {
  ...
  const scenarioObserverSubscription$ = ko.pureComputed(function() {
    ...
    if (!isEmpty(config.scenarioList) && !isEmpty(config.columnOptions)) {
      try {
        error$(undefined);
        var scenarioObserver = insight
          .getView()
          .withScenarios(config.scenarioList)
          .withEntities(getAutoTableEntities(config.columnOptions));

        scenarioObserver = reduce(
          filter(config.columnOptions, ({ remoteFilters }) => !isEmpty(remoteFilters)),
          (observer, { name, remoteFilters }) => observer.filter(name, () => remoteFilters),
          scenarioObserver
        );

        return scenarioObserver
